...
import com.future.saf.sample.rocketmq.biz.mapper.TransferRecordMapper;
import com.future.saf.sample.rocketmq.biz.mapper.UserMapper;
import com.future.saf.sample.rocketmq.exception.BizException;
...
public class BusinessService {
	...
	public boolean doTransfer(Long fromUserId, Long toUserId, Long changeMoney, String businessNo, String transactionId)
		...
		// 执行A扣钱操作
		// update user set money = money - #{money} where id = #{userId} and money >=
		// #{money}
	...
	public boolean checkTransferStatus(String transactionId) {
		...
		int count = transferRecordMapper
				.selectCount(new QueryWrapper<>(new TransferRecord().setTransactionId(transactionId)));
		return count > 0;
